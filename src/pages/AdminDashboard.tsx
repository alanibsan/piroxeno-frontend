...
function getLabels(lang: Lang) {
  ...
  return {
    ...
    metricHints: {
      conversations: es ? "Total de sesiones iniciadas en el periodo filtrado." : "Total sessions started in the selected period.",
      messages: es ? "Total de mensajes enviados por usuarios y assistant." : "Total messages sent by users and assistant.",
      assistant: es ? "Mensajes generados por el assistant." : "Messages generated by the assistant.",
      user: es ? "Mensajes enviados por usuarios finales." : "Messages sent by end users.",
      tokens: es ? "Consumo total de tokens registrado." : "Total recorded token usage.",
      latency: es ? "Tiempo medio de respuesta del assistant." : "Average assistant response time.",
    },
  };
}
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          <p className="flex items-center gap-1.5 text-xs font-semibold uppercase tracking-[0.16em] text-slate-500">
            {label}
            {hint && <span className="inline-flex cursor-help" title={hint} aria-label={hint}><CircleHelp className="h-3.5 w-3.5 text-slate-500" /></span>}
          </p>
...
function PortalLoading({ show }: { show: boolean }) {
  if (!show) return null;
  return (
    <div className="pointer-events-none fixed right-5 top-5 z-50 flex items-center gap-3 border border-white/10 bg-slate-950/80 px-4 py-3 text-sm font-semibold text-slate-200 shadow-2xl shadow-black/30 backdrop-blur-xl">
      <img src="/favicon.png" alt="" className="h-8 w-8 animate-spin" />
      <span>Loading</span>
    </div>
  );
}
...
export default function AdminDashboard() {
  ...
  return (
    <div className={`min-h-screen ${theme === "dark" ? "bg-[#050711] text-white" : "bg-[#f6f7f4] text-slate-950 portal-light"}`}>
      <PortalLoading show={loading || saving} />
        ...
        <main className="px-5 py-5 lg:px-8">
          ...
          {(section === "overview" || section === "conversations") && (
            <div className="mb-5 grid gap-3 md:grid-cols-[1fr_160px_160px] xl:grid-cols-[280px_180px_180px_auto]">
              ...
              <button onClick={() => { setDateStart(""); setDateEnd(""); }} className="border border-white/10 px-3 py-2 text-xs font-semibold text-slate-300 hover:border-white/25">Reset</button>
            </div>
          )}
            ...
            <div className="space-y-6">
              ...
              <section className="border border-white/10 bg-white/[0.035] p-5">
                ...
                <div className="flex h-56 items-end gap-1 border-b border-white/10 pb-2">
                  {activity30d.map((day) => (
                    <div key={day.date} className="group flex min-w-0 flex-1 items-end">
                      <div
                        className="w-full bg-[var(--color-primary)]/80 transition hover:bg-[var(--color-primary)]"
                        style={{ height: `${Math.max(day.messages ? 8 : 2, (day.messages / maxDailyMessages) * 100)}%` }}
                        title={`${day.date}: ${formatNumber(day.messages)} mensajes · ${formatNumber(day.tokens)} tokens`}
                      />
                    </div>
                  ))}
                  {!activity30d.length && <p className="mb-8 w-full text-center text-sm text-slate-500">{loading ? "Loading..." : t.noData}</p>}
                </div>
              </section>
          ...
          {section === "users" && isAdmin && (
            <div className="grid gap-5 xl:grid-cols-[0.75fr_1.25fr]">
              <section className="border border-white/10 bg-white/[0.035] p-5"><h2 className="mb-5 text-xl font-semibold">{t.createUser}</h2><div className="space-y-3"><div className="grid grid-cols-3 border border-white/10 bg-slate-950 p-1 text-sm">{(["existing", "new", "global"] as const).map((mode) => <button key={mode} type="button" onClick={() => { setUserClientMode(mode); if (mode !== "global") setUserForm({ ...userForm, role: "user" }); }} className={`px-3 py-2 font-semibold ${userClientMode === mode ? "bg-[var(--color-primary)] text-slate-950" : "text-slate-400 hover:text-white"}`}>{mode === "existing" ? t.existingClient : mode === "new" ? t.newClient : t.global}</button>)}</div>{userClientMode === "existing" && <Select className="w-full" value={userForm.client_slug || ""} onChange={(e) => setUserForm({ ...userForm, client_slug: e.target.value })}><option value="">{t.selectClient}</option>{clients.map((client) => <option key={client.client_slug} value={client.client_slug}>{client.client_slug}</option>)}</Select>}{userClientMode === "new" && <div className="space-y-3 border border-white/10 bg-slate-950 p-3"><TextInput className="w-full" placeholder="client_slug" value={newUserClientForm.client_slug} onChange={(e) => setNewUserClientForm({ ...newUserClientForm, client_slug: e.target.value })} /><TextInput className="w-full" placeholder={lang === "es" ? "Nombre" : "Name"} value={newUserClientForm.name} onChange={(e) => setNewUserClientForm({ ...newUserClientForm, name: e.target.value })} /><textarea className="w-full border border-white/10 bg-[#050711] p-4 font-mono text-sm text-white" rows={4} placeholder="https://cliente.com" value={newUserClientForm.allowed_origins} onChange={(e) => setNewUserClientForm({ ...newUserClientForm, allowed_origins: e.target.value })} /></div>}{userClientMode === "global" ? <Select className="w-full" value={userForm.role} onChange={(e) => setUserForm({ ...userForm, role: e.target.value as "admin" | "user" })}><option value="user">user</option><option value="admin">admin</option></Select> : <div className="border border-white/10 bg-slate-950 px-4 py-3 text-sm text-slate-300">{t.role}: user</div>}<TextInput className="w-full" placeholder="email@cliente.com" value={userForm.email} onChange={(e) => setUserForm({ ...userForm, email: e.target.value })} /><TextInput type="password" className="w-full" placeholder={t.tempPassword} value={userForm.password || ""} onChange={(e) => setUserForm({ ...userForm, password: e.target.value })} /><label className="flex items-center gap-3 text-sm text-slate-300"><input type="checkbox" checked={userForm.is_active} onChange={(e) => setUserForm({ ...userForm, is_active: e.target.checked })} /> {t.active}</label><button onClick={saveUser} disabled={saving} className="inline-flex items-center gap-2 bg-[var(--color-primary)] px-5 py-3 font-semibold text-slate-950 disabled:opacity-60"><Users className="h-4 w-4" />{t.saveUser}</button></div></section>
